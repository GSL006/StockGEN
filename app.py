...
# Scale the volatility up more aggressively (adjust this factor based on your data)
if last_volatility < 1:
    scaled_volatility = last_volatility * 50  
else:
    scaled_volatility = last_volatility

# Adjust predicted change based on volatility
if predicted_change_scaled > 0:
    predicted_change_scaled_with_volatility = predicted_change_scaled + scaled_volatility
else:
    predicted_change_scaled_with_volatility = predicted_change_scaled - scaled_volatility

# Now directly add the adjusted predicted change to the unscaled last closing price
